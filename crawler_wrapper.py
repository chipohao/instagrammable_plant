...
if __name__ == "__main__":
    ...
    print(config['HASHTAG']['hashtags'])
    hashtags=config['HASHTAG']['hashtags'].split('#')
    print(hashtags)
    #exit()
    if args.mode in ["posts", "posts_full"]:
        ...
        for i in range(len(hashtags)):
            output(
                get_posts_by_hashtags(hashtags[i], args.number or 100, args.debug), './output_folder/{}.txt'.format(hashtags[i])
            )
    else:
        usage()
